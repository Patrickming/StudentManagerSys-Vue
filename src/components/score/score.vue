<template>
  <div>
    <el-card class="cardStyle" v-loading="loading">
      ...
      <el-button @click="batchMethod" type="info"  :disabled="dataTable.length <= 0" style="margin-bottom: 15px;font-size: 18px;" v-if="userInfo.level !== 2"> 批量编辑</el-button>
      <el-button @click="addEntry" type="primary"  :disabled="dataTable.length <= 0" style="margin-bottom: 15px;font-size: 18px;" v-if="userInfo.level !== 2"> 成绩录入</el-button>
      <el-button @click="exportMethod" type="success" :disabled="dataTable.length <= 0"  style="margin-bottom: 15px; font-size: 18px;"> 导出</el-button>
      
                <el-button type="primary" @click="clickSure" size="big" style="margin-right: 10px;font-size: 18px;"> 查询</el-button>
      <!--过滤框-->
...
<script>
  ...
  export default {
    ...
    methods : {
      ...
      getDefault () {
        // 获取学年和学期的初始值
        this.form.year = this.yearArr[0].value;
